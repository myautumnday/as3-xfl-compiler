package nid.xfl.compiler.swf.tags
{
	...
	public class TagDebugID implements ITag
	{
		...
		public function toString(indent:uint = 0):String {
			...
			if (_uuid.length == 16) {
				str += StringUtils2.printf("%02x%02x%02x%02x-", _uuid[0], _uuid[1], _uuid[2], _uuid[3]);
				str += StringUtils2.printf("%02x%02x-", _uuid[4], _uuid[5]);
				str += StringUtils2.printf("%02x%02x-", _uuid[6], _uuid[7]);
				str += StringUtils2.printf("%02x%02x-", _uuid[8], _uuid[9]);
				str += StringUtils2.printf("%02x%02x%02x%02x%02x%02x", _uuid[10], _uuid[11], _uuid[12], _uuid[13], _uuid[14], _uuid[15]);
			} else {
