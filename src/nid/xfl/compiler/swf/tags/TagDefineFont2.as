package nid.xfl.compiler.swf.tags
{
	...
	public class TagDefineFont2 extends TagDefineFont implements IDefinitionTag
	{
		...
		public function set codeTable(value:Vector.<uint>):void { _codeTable = value; }
		public function get fontAdvanceTable():Vector.<int> { return _fontAdvanceTable; }
		public function set fontAdvanceTable(value:Vector.<int>):void { _fontAdvanceTable = value; }
		public function get fontBoundsTable():Vector.<SWFRectangle> { return _fontBoundsTable; }
		public function set fontBoundsTable(value:Vector.<SWFRectangle>):void { _fontBoundsTable = value; }
		public function get fontKerningTable():Vector.<SWFKerningRecord> { return _fontKerningTable; }
		public function set fontKerningTable(value:Vector.<SWFKerningRecord>):void {_fontKerningTable = value; }
		...
		override protected function toStringCommon(indent:uint):String {
			...
			if (hasLayout) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "Ascent: " + ascent;
				str += "\n" + StringUtils2.repeat(indent + 2) + "Descent: " + descent;
				str += "\n" + StringUtils2.repeat(indent + 2) + "Leading: " + leading;
			}
			if (_codeTable.length > 0) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "CodeTable:";
				for (i = 0; i < _codeTable.length; i++) {
					if ((i & 0x0f) == 0) {
						str += "\n" + StringUtils2.repeat(indent + 4) + _codeTable[i].toString();
					} else {
			...
			if (_fontAdvanceTable.length > 0) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "FontAdvanceTable:";
				for (i = 0; i < _fontAdvanceTable.length; i++) {
					if ((i & 0x07) == 0) {
						str += "\n" + StringUtils2.repeat(indent + 4) + _fontAdvanceTable[i].toString();
					} else {
			...
			if (_fontBoundsTable.length > 0) {
				...
				if (hasNonNullBounds) {
					str += "\n" + StringUtils2.repeat(indent + 2) + "FontBoundsTable:";
					for (i = 0; i < _fontBoundsTable.length; i++) {
						str += "\n" + StringUtils2.repeat(indent + 4) + "[" + i + "] " + _fontBoundsTable[i].toString();
					}
				}
			}
			if (_fontKerningTable.length > 0) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "KerningTable:";
				for (i = 0; i < _fontKerningTable.length; i++) {
					str += "\n" + StringUtils2.repeat(indent + 4) + "[" + i + "] " + _fontKerningTable[i].toString();
				}
			}
			return str;
		}
