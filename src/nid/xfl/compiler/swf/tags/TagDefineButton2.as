package nid.xfl.compiler.swf.tags
{
	...
	import nid.xfl.compiler.swf.data.SWFButtonCondAction;
	...
	public class TagDefineButton2 implements IDefinitionTag
	{
		...
		public function set characters(value:Vector.<SWFButtonRecord>):void { _characters = value; }
		...
		public function toString(indent:uint = 0):String {
			...
			if (_characters.length > 0) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "Characters:";
				for (i = 0; i < _characters.length; i++) {
					str += "\n" + StringUtils2.repeat(indent + 4) + "[" + i + "] " + _characters[i].toString(indent + 4);
				}
			}
			if (_condActions.length > 0) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "CondActions:";
				for (i = 0; i < _condActions.length; i++) {
					str += "\n" + StringUtils2.repeat(indent + 4) + "[" + i + "] " + _condActions[i].toString(indent + 4);
				}
			}
			return str;
		}
