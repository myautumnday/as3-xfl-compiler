package nid.xfl.compiler.swf.data
{
	...
	import nid.xfl.compiler.swf.data.consts.GradientInterpolationMode;
	import nid.xfl.compiler.swf.data.consts.GradientSpreadMode;
	import nid.xfl.compiler.swf.data.consts.LineCapsStyle;
	import nid.xfl.compiler.swf.data.consts.LineJointStyle;
	import nid.xfl.compiler.swf.data.etc.CurvedEdge;
	import nid.xfl.compiler.swf.data.etc.IEdge;
	import nid.xfl.compiler.swf.data.etc.StraightEdge;
	import nid.xfl.compiler.swf.exporters.core.DefaultShapeExporter;
	import nid.xfl.compiler.swf.exporters.core.IShapeExporter;
	import nid.xfl.compiler.swf.utils.ColorUtils;
	...
	public class SWFShape
	{
		...
		public function set records(value:Vector.<SWFShapeRecord>):void { _records = value; }
		...
		public function toString(indent:uint = 0):String {
			var str:String = "\n" + StringUtils2.repeat(indent) + "ShapeRecords:";
			for (var i:uint = 0; i < _records.length; i++) {
				str += "\n" + StringUtils2.repeat(indent + 2) + _records[i].toString(indent + 2);
			}
			return str;
		}
	}
