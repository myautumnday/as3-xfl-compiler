	...
	public class SWFShapeWithStyle extends SWFShape
	{
		...
		override public function toString(indent:uint = 0):String {
			...
			if (_initialFillStyles.length > 0) {
				str += "\n" + StringUtils2.repeat(indent) + "FillStyles:";
				for (i = 0; i < _initialFillStyles.length; i++) {
					str += "\n" + StringUtils2.repeat(indent + 2) + "[" + (i + 1) + "] " + _initialFillStyles[i].toString();
				}
			}
			if (_initialLineStyles.length > 0) {
				str += "\n" + StringUtils2.repeat(indent) + "LineStyles:";
				for (i = 0; i < _initialLineStyles.length; i++) {
					str += "\n" + StringUtils2.repeat(indent + 2) + "[" + (i + 1) + "] " + _initialLineStyles[i].toString();
				}
			}
