package nid.xfl.compiler.swf.data
{
	...
	import nid.xfl.compiler.swf.data.consts.BlendMode;
	...
	public class SWFButtonRecord
	{
		...
		public function toString(indent:uint = 0):String {
			...
			if (placeMatrix && !placeMatrix.isIdentity()) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "Matrix: " + placeMatrix;
			}
			if (colorTransform && !colorTransform.isIdentity()) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "ColorTransform: " + colorTransform;
			}
			if (hasFilterList) {
				str += "\n" + StringUtils2.repeat(indent + 2) + "Filters:"
				for(var i:uint = 0; i < filterList.length; i++) {
					str += "\n" + StringUtils2.repeat(indent + 4) + "[" + i + "] " + filterList[i].toString(indent + 4);
				}
			}
			return str;
		}
