		...
		override public function toString(indent:uint = 0):String {
			...
			if (stateNewStyles) {
				...
				if (_fillStyles.length > 0) {
					str += "\n" + StringUtils2.repeat(indent + 2) + "New FillStyles:";
					for (i = 0; i < _fillStyles.length; i++) {
						str += "\n" + StringUtils2.repeat(indent + 4) + "[" + (i + 1) + "] " + _fillStyles[i].toString();
					}
				}
				if (_lineStyles.length > 0) {
					str += "\n" + StringUtils2.repeat(indent + 2) + "New LineStyles:";
					for (i = 0; i < _lineStyles.length; i++) {
						str += "\n" + StringUtils2.repeat(indent + 4) + "[" + (i + 1) + "] " + _lineStyles[i].toString();
					}
				}
			}
			return str;
